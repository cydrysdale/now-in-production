Below is a **V3 draft** incorporating the V2 copyedit review notes (heading capitalization standardized; location normalization split into distinct rows; evidence lists standardized A–L; reduced anthropomorphism; tool-connected guardrail added; token defined in the glossary).

---

# How AI Works (Without the Hype): Marketing Potluck Project Management Edition (V3)
...
> **TL;DR**
> Think of a text AI model as a fast helper that **predicts the next most likely text** (token by token) based on what you give it. It doesn’t “know” your office situation. Your prompt is the difference between “help!” and “here’s the checklist, the rules, and the format.”

---

## Micro-Glossary: Six Terms You’ll See a Lot
...
> A safer mental model: it only uses what’s **in the provided context**, and it may **weight** some parts (instruction-like text, repeated constraints) more than others.
...
**Kid version:** If you tell a coworker “handle the potluck,” they’ll respond with random ideas because you didn’t say what “handle” means.
...
**Reader checkpoint:** The single biggest risk in this scenario isn’t “messy planning.” It’s **safety/policy failure** caused by guessed details.

---

## 2) What the Model Is Doing When It Responds

**Kid version:** It’s like super-fast autocomplete that’s read a lot of writing.

**Work version:** A text model produces responses by repeatedly predicting the **next most likely token** (text piece) given what it can see in the prompt (instructions + context). It can:
...
One line that prevents 80% of workplace confusion:
...
**Kid version:** It reads in small chunks, not whole paragraphs the way you do.
...
**Reader checkpoint:** “Next Thursday” is short, but operationally useless without a calendar date.

---

## 4) Attention and Weighting: Why Placement and Labels Matter

**Kid version:** If the rules are buried, people miss them.

**Work version:** The model is more likely to comply with **instruction-like text** and **repeated constraints** than buried details inside long context. Practical prompt hygiene:
...
**Kid version:** If you dump too much on the table, things fall off.
...
**Kid version:** “Don’t decorate yet—just make the checklist.”

**Work version:** Non-goals reduce two failure modes:

1. **Scope creep**: generating themes, menus, copy you didn’t ask for.
...
**Kid version:** It’s like posting the rules: “Every dish must have a label.”
...
A subtle but important improvement: “No duplicates” can cause over-merging. Better: **merge only clear duplicates** and note uncertainty (“Possible duplicate…”) rather than forcing false consolidation.

---

## 8) Structured Output: Turning the Dump Into a Dataset

**Kid version:** A dataset is a neat table where every row has the same fields.
...
|  2 | Schedule         | Resolve actual calendar date for “next Thursday.”                                      | A, C, L    | Unassigned | Unknown | Tara confirmation                  | Wrong date communicated                     | Yes                 | Med        | “Next Thursday” unspecified    |
...
|  5 | Location         | Choose final location (Kitchen vs Conf Room B vs other approved space); Conf Room B may be booked; Kitchen is in Invite #1. | C, D, L | Unassigned | Unknown | Tara decision + Facilities         | People go to wrong room; capacity/safety issues | Yes             | Med        | None                           |
|  6 | Safety           | Severe nut allergy: define nut-free policy + cross-contamination handling.             | A, F, L    | Unassigned | Unknown | Raj guidance + comms owner         | Health risk                                 | Yes                 | High       | None                           |
|  7 | Policy           | HR: participation voluntary; avoid pressure; no alcohol at lunch.                      | H          | Unassigned | Unknown | Messaging owner                    | “Mandatory fun” backlash; policy violation  | No                  | High       | None                           |
...
| 10 | Labeling         | Create label template for allergens + heat level.                                      | A, D, F    | Becky      | Unknown | None noted                         | Allergy/complaints if missing               | No                  | High       | Becky already has template      |
| 11 | Labeling         | Enforce labeling + update sign-up doc fields to require allergens + heat level.        | A, D, F    | Unassigned | Unknown | DRI + Becky template               | Labeling inconsistency                      | Yes                 | High       | None                           |
| 12 | Exec requirement | Ensure at least one gluten-free + dairy-free “safe” option; clarify “Whole30-ish.”     | K          | Unassigned | Unknown | Exec clarification                 | Exec dissatisfaction; dietary mismatch      | Yes                 | Med        | “Whole30-ish” undefined         |
| 13 | Source of truth  | Locked spreadsheet attachment blocks info; request access or alternate source.         | C          | Unassigned | Unknown | Access owner                       | Missing logistics                           | Yes                 | High       | File access controlled          |
| 14 | Menu conflict    | “Dessert (almond flour?)” conflicts with nut allergy constraint.                       | A, F       | Unassigned | Unknown | Nut policy decision                | Allergy risk                                | Yes                 | High       | Almond flour implies tree nut   |
...
**Kid version:** AI can help you make the checklist, but it can’t decide what tastes good.
...
**Kid version:** Writing “bake at 350°F” doesn’t turn on the oven.
...
If a system *is* tool-connected, require it to cite what it retrieved (source + timestamp) or explicitly mark the item unknown. Even with tools, treat outputs as **reports** of what was retrieved (and when), not guarantees. Otherwise, you get “it said it checked the calendar” without verifiable evidence.

---

## 11) Reliability: QA the Plan Like a Launch Checklist

Treat model output like a first draft from a junior coordinator: useful, but needs QA.
...
* **Source integrity:** Every normalized row points back to at least one evidence label (A–L).
